...
class DragTarget {
  ...
  moveTo (x, y) {
    const snap = parseInt(document.body.dataset._webedit_snap, 10)
    if (snap) {
  ...
  setWidth (width) {
    ...
    this.element.style.width = Math.max(width, 24) + 'px'
  }
  ...
  setHeight (height) {
    ...
    this.element.style.height = Math.max(height, 24) + 'px'
  }
}
...
class RightEdgeDragStrategy extends EdgeDragStrategy {
  ...
  onmove (targets, dx, _dy) {
    for (const target of targets) {
      target.setWidth(target.offsetWidth + dx)
    }
  }
}
...
class BottomEdgeDragStrategy extends EdgeDragStrategy {
  ...
  onmove (targets, _dx, dy) {
    for (const target of targets) {
      target.setHeight(target.offsetHeight + dy)
    }
  }
}
...
class DragHandler {
  ...
  move (context) {
    ...
    if (this.strategy !== this.noopStrategy) {
      this.targets.forEach(target => this.onchange(target.element))
    }
  }
}
...
class KeyHandler {
  ...
  onupdate () {
    this.selection.forEach(element => {
      const target = DragTarget.create(element)
      target.moveBy(this.dx, this.dy)
